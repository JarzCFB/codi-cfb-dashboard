...
def norm_name(s):
    ...
    s = re.sub(r"\([^)]*\)", "", s)
    s = re.sub(r"[^a-z0-9]", "", s)
    return s.replace("university", "").replace("state", "st")
    ...
    for g in data:
        ...
        for bk in g.get("bookmakers",[]):
            for market in bk.get("markets",[]):
                ...
                for selection in market.get("outcomes",[]):
...
if ok_odds:
    try:
        ...
        if not df.empty:
            ...
            with tab2:
                best=df.sort_values(["Matchup","Team","Expected ROI"],ascending=[True,True,False]).drop_duplicates(["Matchup","Team"])
